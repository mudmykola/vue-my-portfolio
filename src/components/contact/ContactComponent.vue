...
const contactSubTitleItem = ref([
  {id: 1, text: ' Get in Touch'}
])
</script>

<template>
  <div class="contact-box top-[11%] left-[12%] container-centered  animate__animated animate__backInDown">
      ...
      <div class="contact-title  flex flex-col  items-center">
        <div class="contact-title__box flex flex-col items-end">
          ...
          <div class=" contact-subtitle flex text-center justify-center w-fit">
            <p class="text-[15px]" v-for="item in contactSubTitleItem" :key="item.id">{{ item.text }}</p>
          </div>
        </div>
      </div>
