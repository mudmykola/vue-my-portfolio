...
onMounted(async () => {
  try {
    ...
    filters.value = [
      'All',
      ...new Set(data.portfolioCard.map((project) => project.category)),
    ];
...
<template>
  <div class="portfolio-inner">
    ...
    <div class="portfolio-inner__content flex flex-wrap gap-3 justify-center">
      <div
        ...
        :key="project.id"
        class="project-wrapper w-[350px]"
      >
        <div
          class="project-card rounded backdrop-blur"
        ...
        >
          ...
          <p
            class="text-default rounded font-medium mt-2 bg-[#ca0de1] w-fit px-2 py-1"
          >
            {{ project.technology }}
          </p>
        </div>
  ...
  <div v-if="selectedProject" class="modal show">
    <div
      class="modal-content h-full rounded fixed top-0 left-0 right-0 bottom-0 overflow-auto z-50"
    >
      <span class="close" @click="closeModal">&times;</span>
      <div class="modal-wrapper mt-10">
        <swiper class="w-[50%]" :options="swiperOptions">
          <swiper-slide
            v-for="(image, index) in selectedProject.imageSet"
            :key="index"
          >
            <img
        ...
        <div class="project-details">
          ...
          <div class="details-group">
            <div class="details-group__desc">
              <h2 class="modal-details__desc text-2xl font-bold">
                Description:
              </h2>
              <p>{{ selectedProject.descriptionFull }}</p>
            </div>
            <div class="details-group__tech mt-5">
              <h2 class="modal-details__tech text-2xl font-bold">
                Technology:
              </h2>
