...
<template>
  <div class="services mt-10 testimonial">
    ...
    <div class="testimonial-inner mt-5" v-if="store.testimonials.length">
      <Swiper>
        <template
          v-for="(group, index) in store.groupedTestimonials"
          :key="index"
        >
          <SwiperSlide>
            <div class="testimonial-group flex justify-center gap-5">
              <div
                ...
                :key="idx"
                class="testimonial-item rounded py-2 px-2"
              >
                ...
                <div class="testimonial-author flex items-center gap-2 pt-2">
                  <div class="testimonial-author__image">
                    <img
                      width="60"
                      height="60"
                      :src="store.getPhotoUrl(testimonial.photo)"
                      :alt="testimonial.author"
                      class="photo rounded"
                    />
                  </div>
                  <div class="testimonial-author__text">
                    <p class="author text-[18px] font-bold">
                      {{ testimonial.author }}
                    </p>
                    <p class="testimonial-company font-medium rounded">
                      {{ testimonial.company }}
                    </p>
                  </div>
                </div>
